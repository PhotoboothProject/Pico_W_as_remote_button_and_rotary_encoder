...
# Define the main target directory for Photobooth Pi Pico Buzzer
target="Photobooth_Pi_Pico_Buzzer"
...
download_library() {
  ...
  if [[ "$repo_name" == "Adafruit_CircuitPython_HID" ]]; then
    ...
    unzip -oq -j "$asset_file" "**/*.mpy" -d "$target/lib/adafruit_hid"
  else
    # Extract only the specified .mpy file for other libraries
    unzip -oq -j "$asset_file" "**/${mpy_file_name}" -d "$target/lib"
  fi
  ...
  echo "$mpy_file_name: $latest_version" >> "$target/library_info.txt"
}
...
echo "Library Information - Downloaded Libraries and Versions" > "$target/library_info.txt"
echo "------------------------------------------------------" >> "$target/library_info.txt"
...
cat <<EOL > "$target/settings.toml"
CIRCUITPY_WIFI_SSID = "$wifi_ssid"
CIRCUITPY_WIFI_PASSWORD = "$wifi_password"
EOL

echo "Created/Updated $target/settings.toml."
...
curl -s -L -o "$target/code.py" "$code_url"

# Replace placeholders in `code.py` with entered Remotebuzzer Server IP and Port
sed -i "s/Your_Server_IP/$server_ip/g" "$target/code.py"
sed -i "s/Your_Server_Port/$server_port/g" "$target/code.py"

echo "Modified code.py with server IP and port."
echo "################"
echo "Setup complete. All files are located in the $target folder."
